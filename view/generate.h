...
public:
	...
	OEL_ALWAYS_INLINE
	constexpr _generateIterator & operator++()        { --m.first;  return *this; }
	OEL_ALWAYS_INLINE
	constexpr void                operator++(int) &   { --m.first; }
	...
	friend constexpr bool operator==(const _generateIterator & left, _defaultSentinel)   { return left.m.first == 0; }

	friend constexpr bool operator==(_defaultSentinel, const _generateIterator & right)  { return right.m.first == 0; }
...
inline constexpr auto generate =
	[](auto generator, ptrdiff_t count)
	{
	...
	template< typename G >
	inline constexpr bool disable_sized_sentinel_for< _defaultSentinel, _generateIterator<G> > = false;
#endif
