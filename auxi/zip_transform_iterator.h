...
class _zipTransformIterator
{
	...
	OEL_NO_UNIQUE_ADDRESS oel::_detail::MakeAssignable<Func> _fn;
...
public:
	...
	template< typename S >
	friend constexpr bool operator==
		(const _zipTransformIterator & left, _sentinelWrapper<S> right)   { return std::get<0>(left._iters) == right.se; }
...
private:
	template< size_t... Ns >
	OEL_ALWAYS_INLINE constexpr reference _apply(std::index_sequence<Ns...>) const
	{
		const Func & f = _fn;
