...
{
	using _iter = iter_transform_iterator< Func, iterator_t<View> >;
...
public:
	...
	constexpr _iter begin()
		{
			Func & f = _m.second();
			return {_detail::MoveIfNotCopyable(f), _m.first.begin()};
		}
...
template< typename V, typename F >
inline constexpr bool std::ranges::enable_view< oel::_iterTransformView<V, F> > = true;
