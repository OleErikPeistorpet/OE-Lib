...
// TODO: disable assignment of whole struct when any type is not trivially destructible
...
namespace _detail
{
	...
	struct Zip
	{
		...
		static RvalueStruct IterMove(const std::tuple<Is...> & iters, std::index_sequence<Ns...>) noexcept
		{
			return{ std::ranges::iter_move( std::get<Ns>(iters) )... };
		}
	};
...
auto iter_move(const _zipTransformIterator< oel::_detail::Zip<S, R>, Iterators... > & it) noexcept
	{
		return oel::_detail::Zip<S, R>::IterMove(it.base(), std::index_sequence_for<Iterators...>{});
	}


template< typename T, align_val_t A >
struct field_array<_detail::ViewTag, T, A>
 :	public _countedView<T *>
{};

template< typename T, align_val_t A >
struct field_array<_detail::ConstViewTag, T, A>
 :	public _countedView<const T *>
{};
