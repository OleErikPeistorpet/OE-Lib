...
public:
	using iterator_category = decltype( iter::_transformCategory<true, Func, Iterators...>() );

	using typename _zipTransform::_iteratorFacade::difference_type;
	...
	constexpr _zipTransform & operator++()
		{
			_increment(_iSeq{});  return *this;
		}
	//! Post-increment: return type is _zipTransform if iterator_category is-a forward_iterator_tag, else void
	...
	constexpr _zipTransform   operator--(int) &
		{
			auto tmp = *this;
			_decrement(_iSeq{});
			return tmp;
	...
	constexpr _zipTransform & operator+=(difference_type offset) &
		{
	...
	friend constexpr difference_type operator -(const _zipTransform & left, const _zipTransform & right)
		...
		{
			return std::get<0>(_iters) - std::get<0>(right._iters);
		}
	template< typename S >
