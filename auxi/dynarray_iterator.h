...
public:
	...
	bool operator==(const dynarray_iterator<Ptr2> & right) const
	{
		OEL_ITER_CHECK_COMPATIBLE(right);
		return _pElem == right._pElem;
	}
...
typename std::pointer_traits<Ptr>::element_type *
	to_pointer_contiguous(const dynarray_iterator<Ptr> & it) noexcept  { return it._pElem; }
...
namespace _detail
{
	template< typename Ptr, typename MutPtr >
	dynarray_iterator<Ptr> MakeDynarrayIter(Ptr const pos, MutPtr const begin, const void * container) noexcept
	{
	#if OEL_MEM_BOUND_DEBUG_LVL
		if (begin)
		{
			const auto * h = OEL_DEBUG_HEADER_OF(begin);
			return {pos, h, h->id};
		}
		else
	#endif
		{	return {pos, &_detail::headerNoAllocation, reinterpret_cast<std::uintptr_t>(container)};
		}
	}
}
