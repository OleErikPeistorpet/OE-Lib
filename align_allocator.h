...
///
template<typename... Ts>
using aligned_union_t = typename aligned_union<Ts...>::type;
...
struct aligned_union
{
	static size_t const alignment_value = _detail::Max<OEL_ALIGNOF(Ts)...>::value;
	using type = aligned_storage_t<_detail::Max<sizeof(Ts)...>::value, alignment_value>;
};
...
namespace _detail
{
	...
	void UninitFillA(T *const first, T *const last, Alloc & alloc, const Arg &... arg)
	{
	...
	void UninitFill(T *const first, T *const last, const Arg &... arg)
	{
		allocator<T> a;
		UninitFillA(first, last, a, arg...);
	}
