...
/// Create a counted_range from iterator and size
template<typename Iterator> inline
counted_range<Iterator>  make_range_n(Iterator first, size_t count)  { return {first, count}; }
...
/// Create a counted_range of move_iterator from counted_range
template<typename InputIterator, typename S> inline
counted_range< std::move_iterator<InputIterator>, S >  move_range(const counted_range<InputIterator, S> & r)
	{ return {std::make_move_iterator(r.begin()), count}; }
/// Create a counted_range of move_iterator from iterator and size
template<typename InputIterator>
counted_range< std::move_iterator<InputIterator> >     move_range_n(InputIterator first, size_t count)
	{ return oel::make_range_n(std::make_move_iterator(first), count); }

template<typename T>
counted_range<unsigned char *> as_binary(T & obj)
{
...
template<typename T>
counted_range<const unsigned char *> as_binary(const T & obj)
{
	static_assert(is_trivially_copyable<T>::value, "Unsafe");
